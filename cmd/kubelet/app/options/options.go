...
func AddKubeletConfigFlags(mainfs *pflag.FlagSet, c *kubeletconfig.KubeletConfiguration) {
	...
	fs.Int32Var(&c.MaxContainerBackOffSeconds, "max-container-backoff-seconds", c.MaxContainerBackOffSeconds, "Max backoff period in seconds, for user control. If not set, use 300s as default value")
	fs.Int32Var(&c.ContainerBackOffResetSeconds, "container-backoff-reset-seconds", c.ContainerBackOffResetSeconds, "Reset period of a success restart of pod. If not set, use 2 * MaxContainerBackOffSeconds as default value")
	fs.Int32Var(&c.NodeLeaseRenewIntervalSeconds, "node-lease-renew-interval-seconds", c.NodeLeaseRenewIntervalSeconds, "Node lease renew interval, default 10s.")
}
